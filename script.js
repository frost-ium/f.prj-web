document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateGradient() {
        hue = (hue + 1) % 360;
        const color1 = `hsl(${hue}, 100%, 20%)`;
        const color2 = `hsl(${(hue + 60) % 360}, 100%, 20%)`;
...
// Frostium hover effect
const frostium = document.querySelector('.frostium');
const body = document.body;

if (frostium) {
	frostium.addEventListener('mouseenter', () => {
		body.style.transition = 'background-color 0.5s ease';
		body.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
	});

	frostium.addEventListener('mouseleave', () => {
		body.style.backgroundColor = '';
	});
}

// Önceki animasyonları kaldır
